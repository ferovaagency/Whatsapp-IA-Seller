...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${evolutionUrl}/instance/create`, {
      ...
      body: JSON.stringify({ instanceName: client.instance_name, qrcode: true, integration: "WHATSAPP-BAILEYS" }),
    ...
    if (!res.ok) {
      return NextResponse.json({ error: result?.response?.message?.[0] ?? "Error al crear instancia" }, { status: 500 });
    }

    const appUrl = (process.env.NEXT_PUBLIC_APP_URL || "").trim();
    if (appUrl) {
      fetch(`${evolutionUrl}/webhook/set/${client.instance_name}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", apikey: evolutionKey },
    ...
    return NextResponse.json({ ok: true, instance: client.instance_name });
  } catch (err) {
